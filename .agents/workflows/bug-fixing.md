...
1. Capture expected versus actual behavior and localize the fault in
   `docs/current/spec_v<N>.md`.
...
3. Write `implementation_plan_v<N>.md` with the root cause, minimal fix, and
   regression evidence. Stop for user approval before changing production code.
