...
Use this for an ad hoc, single-slice endpoint, domain capability, persistence
change, or meaningful enhancement. For a complex feature that needs a parent
plan and multiple slices, use `harness-planning.md` instead.

Pipeline: context → implementation and test plan → user
...
3. Complete the implementation and test plans in the same
   ...
   verification matrix and generated test cases from this feature's implementation
   plan and acceptance criteria using the Go testing skill. The test-plan template records
...
7. Update `docs/product/product.md` and add a durable note under `docs/changes/`
